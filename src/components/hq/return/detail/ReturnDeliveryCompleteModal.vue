<template>
      ...
      <div class="modal-content">
        ...
        <div v-if="!isConfirmStep">
          ...
          <div class="modal-body">
            ...
            <div class="input-group">
              <label>배송 기사명</label>
              <input type="text" v-model="form.driverName" placeholder="배송 기사 성함">
            </div>
            <div class="input-group">
              <label>기사 연락처</label>
              <input type="text" v-model="form.driverPhone" placeholder="예: 010-1234-5678">
            </div>
            <div class="input-group">
              <label>운송장 번호</label>
          ...
          <div class="modal-body">
            ...
            <div class="confirm-info">
              ...
              <div class="info-row">
                ...
                <span class="value">{{ form.driverName }}</span>
              </div>
              <div class="info-row">
                <span class="label">기사 연락처:</span>
                <span class="value">{{ form.driverPhone }}</span>
              </div>
              <div class="info-row">
  ...
  const props = defineProps({ 
    visible: Boolean, 
    returnData: Object  // prop 이름을 returnData로 변경하여 info card와 일치시킴
  });
  ...
  const form = ref({
    deliveryCompany: '',
    driverName: '',      // name → driverName으로 변경
    driverPhone: '',     // phone → driverPhone으로 변경
    trackingNumber: ''
  });
  
  watch(
    () => [props.visible, props.returnData],
    ([visible, returnData]) => {
      if (visible && returnData) {
        form.value = { 
          deliveryCompany: returnData.deliveryCompany || '',
          driverName: returnData.driverName || '',
          driverPhone: returnData.driverPhone || '',
