...
const tabs = [
  { key: 'supplier-info', label: '공급처 정보', desc: '공급처의 기본 정보와 계약 현황을 확인하고 관리할 수 있습니다.' },
  { key: 'delivery-info', label: '납품관리', desc: '해당 공급처의 납품 현황을 관리합니다.' },
  { key: 'purchase-order', label: '발주관리', desc: '해당 공급처의 발주 현황을 확인하고 관리할 수 있습니다.' },
];
...
watch(
  ...
  async (newCompany) => {
    if (newCompany && newCompany.id) {
      try {
        const { data } = await api.get(`/api/hq/suppliers/detail/${newCompany.id}`);
        supplier.value = data;
      } catch (e) {
      ...
      <div class="info-group">
        ...
        <div class="content-area">
          <div v-if="activeTab === 'supplier-info'">
            <InfoForm v-if="selectedSupplierId" :supplierId="selectedSupplierId" />
          ...
          <div v-else-if="activeTab === 'delivery-info'" class="p-8 text-center text-gray-500">
            납품관리 기능은 현재 준비중입니다.
          </div>
        </div>
      </div>
