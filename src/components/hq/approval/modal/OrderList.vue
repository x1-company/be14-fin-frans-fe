    ...
    <div class="modal-card">
      ...
      <div class="orderlist-table-container">
        <div class="orderlist-table-header">
          ...
          <div class="col-date">주문일</div>
          <div class="col-store">가맹점명</div>
          <div class="col-amount">금액</div>
        </div>
        <div v-if="loading" class="orderlist-loading"
          >데이터를 불러오는 중...</div
        >
        <div v-else-if="orderData.length === 0" class="orderlist-no-data"
          >주문 데이터가 없습니다.</div
        >
        <div v-else>
          <div
...
const selectAllRows = () => {
  if (selectedItems.value.length === orderData.value.length) {
    selectedItems.value = [];
  } else {
    selectedItems.value = orderData.value.map((item) => item.code);
  }
};
...
const handleRegister = () => {
  const selectedDocuments = orderData.value.filter((item) =>
