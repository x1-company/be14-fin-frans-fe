...
const getDocumentTypeClass = (code) => {
  if (code?.startsWith("COOP")) return "type-cooperation";
...
const getDocumentTypeText = (code) => {
  if (code?.startsWith("COOP")) return "협조서";
...
.document-icon.type-cooperation {
  background: #e8f5e9;
  color: #2e7d32;
}
